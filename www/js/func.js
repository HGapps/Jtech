  				...
  				if ($new_favs !== null) {
  					$.each($new_favs, function(i, fav) {
  						...
  						$.getJSON(url, function(tick) {
  							...
  							$(".favs table tbody")
  								.append('<tr><td><a href="#" data-link="' + fav + '" class="ticket_link" onclick="notify(' + fav + ')">' + tick['ticket'].title + '</a></td><td><span class = "btn btn-warning" > ' + tick['ticket'].status + '</span></td></tr> ');
  						});
  					});
  				}
  			}
